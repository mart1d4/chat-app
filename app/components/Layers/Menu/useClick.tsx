...
export function useClick(context: FloatingRootContext, props: UseClickProps = {}): ElementProps {
    ...
    const {
        ...
        stickIfOpen = true,
        setPositionToCursor = false,
    } = props;

    // @ts-ignore - `reference` is a custom prop
    const pointerTypeRef = useRef<"mouse" | "pen" | "touch">();
    const didKeyDownRef = useRef(false);

    function setElementPositionToCursor(event: MouseEvent) {
        if (!setPositionToCursor) return;
        ...
        () => ({
            ...
            onMouseDown(event) {
                ...
                if (
                    ...
                    setElementPositionToCursor(event.nativeEvent);
            ...
            onClick(event) {
                ...
                if (eventOption === "mousedown" && pointerTypeRef.current) {
                    // @ts-ignore - `pointerType` is a custom prop
