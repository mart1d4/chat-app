...
export const GuildChannels = ({
    ...
    useEffect(() => {
        ...
        if (!guild.channels.length && !!initChannels.length) {
            updateGuild(guildId, { channels: initChannels });
        }

        if (guild.members.length < initMembers.length && !!initMembers.length) {
            updateGuild(guildId, { members: initMembers });
        }
    }, [guild]);
