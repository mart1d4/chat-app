...
export const Message = ({
  ...
  useEffect(() => {
    ...
    guildInvites.forEach(async (code) => {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/invites/${code}`,
        {
          method: "GET",
        }
      );
      ...
      if (!data.invite && data.invite !== null) {
        // @ts-expect-error
        return setInvites((invites) => [
          ...invites,
          { code: code, type: "error" },
        ]);
      }
  ...
  const retrySendMessage = async (prevMessage: TMessage) => {
    ...
    try {
      if (prevMessage.attachments.length > 0) {
        const onProgress = (
          props: ComputableProgressInfo | UnknownProgressInfo
        ) => {
  ...
  const setLocalStorage = (data: {}) => {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...JSON.parse(
          localStorage.getItem(`channel-${message.channelId}`) || "{}"
        ),
        ...data,
      })
    );
                ...
                {message.messageReference && (
                  <span
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (
                        layers.USER_CARD?.settings.element === e.currentTarget
                      ) {
                        setLayers({
                          settings: {
                            type: "USER_CARD",
